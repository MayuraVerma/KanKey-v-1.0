...
BOOL EXPORTED_DLL_FUNCTION InstallKeyboardHookHemavathi(HWND hWnd) 
{
   BOOL bSuccess = FALSE;
    if( !ghKeyHook) 
	{
...
long __stdcall InstallKeyboardHookHemavathiDEF()
{
   
   // InstallKeyboardHookHemavathi(hwnd);
		glpfnHookProc = (HOOKPROC) SetKeyBoardHemavathi;
...
long __stdcall DeInstallKeyboardHookHemavathiDEF()
{
    DeInstallKeyboardHookHemavathi();
	return TRUE;
}
//---------------Un-Insatalls-the-Hook---//---This-is -called-by-the-.exe---//
BOOL EXPORTED_DLL_FUNCTION DeInstallKeyboardHookHemavathi()
{
	if(ghKeyHook) 
	{
