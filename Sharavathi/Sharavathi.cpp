...
BOOL EXPORTED_DLL_FUNCTION InstallKeyboardHookSharavathi(HWND hWnd) 
{
   BOOL bSuccess = FALSE;
    if( !ghKeyHook) 
	{
...
long __stdcall InstallKeyboardHookSharavathiDEF() 
{
   // InstallKeyboardHookSharavathi(hwnd);
...
long __stdcall DeInstallKeyboardHookSharavathiDEF() 
{
    DeInstallKeyboardHookSharavathi();
	return TRUE;
}
//---------------Un-Insatalls-the-Hook---//---This-is -called-by-the-.exe---//
BOOL EXPORTED_DLL_FUNCTION DeInstallKeyboardHookSharavathi()
{
	if(ghKeyHook) 
	{
